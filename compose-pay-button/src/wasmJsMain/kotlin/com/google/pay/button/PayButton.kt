...
@JsFun("""(onClick, allowedPaymentMethodsJson, buttonColor, buttonType, radiusPx) => {
    const paymentsClient = new google.payments.api.PaymentsClient({environment: 'TEST'});
    const allowedPaymentMethods = JSON.parse(allowedPaymentMethodsJson);
    ...
    button.style.borderRadius = radiusPx;
    button.style.width = '100%';
    button.style.height = '100%';
    return button;
}""")
private external fun createGooglePayButton(
    onClick: () -> Unit,
    ...
    when (val state = sdkState) {
        ...
        SdkState.Ready -> {
            DisposableEffect(theme, type, radius, allowedPaymentMethods) {
                ...
                val button = createGooglePayButton(
                    onClick = onClick,
                    allowedPaymentMethodsJson = allowedPaymentMethods,
                    buttonColor = buttonColor,
                    buttonType = buttonType,
                    radiusPx = radiusPx
                )
